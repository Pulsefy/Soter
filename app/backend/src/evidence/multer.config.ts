...
export const evidenceUploadOptions: MulterOptions = {
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (!isValidFilename(file.originalname)) {
      cb(
        new BadRequestException('Invalid filename. Filenames must be safe and not contain path traversal characters.'),
        false,
      );
      return;
    }
 ...
 */
export function handleMulterError(error: any): any {
  if (error.code === 'LIMIT_FILE_SIZE') {
    return new BadRequestException(`File size exceeds maximum limit of ${MAX_FILE_SIZE / 1024 / 1024}MB`);
  }
  ...
  if (error.code === 'LIMIT_UNEXPECTED_FILE') {
    return new BadRequestException('Unexpected file field. Use "file" as the field name');
  }
