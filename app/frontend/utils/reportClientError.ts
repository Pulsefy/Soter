...
export async function reportClientError(error: Error, correlationId?: string) {
  ...
  try {
    ...
    // Use sendBeacon or fetch to report error asynchronously without blocking UI unmounts
    ...
    // Fail silently to prevent recursive crash loops or surfacing errors to users
    console.error('Failed to report client error:', reportingErr);
  }
}

function sanitizeErrorMessage(msg: string): string {
  // Strip potential PII or email-like strings from error messages
