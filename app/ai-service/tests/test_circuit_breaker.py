...
class TestHumanitarianVerificationServiceCircuitBreaker:
    ...
    def test_verify_claim_skips_provider_when_circuit_open(self, monkeypatch):
        ...
        monkeypatch.setattr(
            self.service, "_provider_attempt_order", lambda pref: ["openai", "groq"]
        )
        monkeypatch.setattr(
    ...
    def test_request_timeout_raises_ai_timeout(self, mock_post, monkeypatch):
        ...
        monkeypatch.setattr(settings, "openai_api_key", "test-key")
        monkeypatch.setattr(
            self.service, "_provider_attempt_order", lambda pref: ["openai"]
        )
        monkeypatch.setattr(
            self.service, "_get_model_for_provider", lambda p: "test-model"
        )
        ...
        assert (
            self.service.breakers["openai"].failure_count == 2
        )  # Primary & fallback attempts both failed
